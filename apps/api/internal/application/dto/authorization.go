...
type RoleResp struct {
	...
	// DataScope 与 DepartmentIDs（仅 CUSTOM 有值）决定成员可见的用户数据
	DataScope     permission.DataScope `json:"dataScope"`
	DepartmentIDs []uint               `json:"departmentIds"`
}

// RoleDataScopeReq 设置角色数据范围请求
type RoleDataScopeReq struct {
	DataScope     permission.DataScope `json:"dataScope" binding:"required,max=32"`
	DepartmentIDs []uint               `json:"departmentIds" binding:"max=500"`
}
