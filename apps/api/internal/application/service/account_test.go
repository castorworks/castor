...
func TestAccountService_PutUserPasswordRevokesSessions(t *testing.T) {
	...
	err := svc.PutUserPassword(context.Background(), 7, &dto.UserPasswordPutReq{CurrentPassword: "old-password", NewPassword: "new-password"})
	if err != nil {
		t.Fatalf("PutUserPassword() error = %v", err)
	}
	if !hash.BcryptMatchPassword("new-password", repo.users["alice@example.com"].Password) {
...
func TestAccountService_PutUserPasswordWrongCurrentKeepsSessions(t *testing.T) {
	...
	err := svc.PutUserPassword(context.Background(), 7, &dto.UserPasswordPutReq{CurrentPassword: "wrong", NewPassword: "new-password"})
...
func TestAccountService_PutUserPasswordByReset(t *testing.T) {
	...
	t.Run("valid code updates password and revokes sessions", func(t *testing.T) {
		t.Parallel()
		svc, repo, rbac := newAccountFixture(t, true)
		err := svc.PutUserPasswordByReset(context.Background(), &dto.UserPasswordResetPutReq{Username: "alice@example.com", ConfirmCode: "123456", NewPassword: "new-password"})
		if err != nil {
	...
	t.Run("invalid code is rejected before user lookup", func(t *testing.T) {
		...
		err := svc.PutUserPasswordByReset(context.Background(), &dto.UserPasswordResetPutReq{Username: "alice@example.com", ConfirmCode: "000000", NewPassword: "new-password"})
	...
	t.Run("unknown user returns the same error as a wrong code", func(t *testing.T) {
		...
		for _, valid := range []bool{true, false} {
			...
			err := svc.PutUserPasswordByReset(context.Background(), &dto.UserPasswordResetPutReq{Username: "nobody@example.com", ConfirmCode: "123456", NewPassword: "new-password"})
			if !errors.Is(err, ErrInvalidConfirmCode) {
