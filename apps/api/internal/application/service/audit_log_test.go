...
func TestAuditLogService_Gets(t *testing.T) {
	...
	logs, total, err := svc.Gets(context.Background(), 1, 3, "created_at DESC")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if total != 5 {
	...
	logs, total, err = svc.Gets(context.Background(), 2, 3, "created_at DESC")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(logs) != 2 {
...
func TestAuditLogService_DeleteBefore(t *testing.T) {
	...
	deleted, err := svc.DeleteBefore(context.Background(), cutoff)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if deleted != 5 {
