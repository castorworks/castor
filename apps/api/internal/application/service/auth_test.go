...
func TestAuthService_ValidatePasswordLength(t *testing.T) {
	tests := []struct {
		name      string
		password  string
		minLength string
		...
		{name: "valid password", password: "password123", minLength: "6", wantErr: false},
		{name: "password too short", password: "abc", minLength: "6", wantErr: true},
		{name: "exact minimum length", password: "abcdef", minLength: "6", wantErr: false},
		{name: "empty password", password: "", minLength: "1", wantErr: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := svc.settingHelper.ValidatePasswordLength(context.Background(), tt.password)
