...
// Login 按登录方式认证用户，并异步记录登录历史与审计日志
func (s *LoginService) Login(ctx context.Context, loginMethod string, req *LoginReq, client LoginClient) (*user.User, error) {
	...
	go s.recordLogin(req.Username, client, loginMethod, err == nil, userID)

	return u, err
}

// recordLogin 记录登录历史与审计日志（不依赖请求生命周期）
...
func (s *LoginService) loginByPassword(ctx context.Context, req *LoginReq) (*user.User, error) {
	...
	if captchaEnabled && !s.policy.Development {
		if !s.captcha.VerifyCaptcha(req.CaptchaId, req.CaptchaCode) {
			log.WarnCtx(ctx).Str("username", req.Username).Msg("Captcha code is invalid")
...
func (s *LoginService) loginByVerificationCode(ctx context.Context, req *LoginReq) (*user.User, error) {
	...
	if required && !s.captcha.VerifyCaptcha(req.CaptchaId, req.CaptchaCode) {
		log.WarnCtx(ctx).Str("username", req.Username).Msg("Captcha code is invalid")
		return nil, ErrInvalidCaptchaCode
	}
