...
func TestLoginHistoryService_Gets(t *testing.T) {
	...
	t.Run("returns paginated results", func(t *testing.T) {
		...
		items, total, err := svc.Gets(context.Background(), 1, 2, "id desc")
		if err != nil {
			t.Fatalf("expected no error, got: %v", err)
		}
		if total != 3 {
	...
	t.Run("returns empty for no data", func(t *testing.T) {
		...
		items, total, err := svc.Gets(context.Background(), 1, 10, "id desc")
		if err != nil {
			t.Fatalf("expected no error, got: %v", err)
		}
		if total != 0 {
	...
	t.Run("filters by username via option", func(t *testing.T) {
		...
		items, total, err := svc.Gets(context.Background(), 1, 10, "id desc", *query.NewOption("username = ?", "alice"))
		if err != nil {
			t.Fatalf("expected no error, got: %v", err)
		}
...
func TestLoginHistoryService_BatchDelete(t *testing.T) {
	...
	t.Run("successful batch delete", func(t *testing.T) {
		...
		err := svc.BatchDelete(context.Background(), []uint{1, 3, 5})
		if err != nil {
			t.Fatalf("expected no error, got: %v", err)
		}
		if len(repo.items) != 2 {
	...
	t.Run("batch delete with error", func(t *testing.T) {
		...
		err := svc.BatchDelete(context.Background(), []uint{1})
		if err == nil {
			t.Fatal("expected error, got nil")
		}
	})
