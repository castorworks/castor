...
func TestUserAvatarService_DownloadRejectsNonAvatarKey(t *testing.T) {
	svc := &userAvatarService{}
	var buf bytes.Buffer

	err := svc.Download(context.Background(), "assets/private/report.pdf", &buf)
	if !errors.Is(err, apperror.ErrAssetNotFound) {
		t.Fatalf("expected ErrAssetNotFound, got %v", err)
	}
	if buf.Len() != 0 {
		t.Fatal("nothing must be written for a rejected key")
	}
}
...
func TestNewRsaService_KeySelection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := NewRsaService(nil, tt.keys).(*rsaService)
	...
	dedicated := NewRsaService(nil, RsaKeyConfig{PrivateKeySecret: "rsa-secret", FallbackSecret: "jwt-key"}).(*rsaService)
	fallback := NewRsaService(nil, RsaKeyConfig{FallbackSecret: "jwt-key"}).(*rsaService)
