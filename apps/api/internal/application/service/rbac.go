...
type RBACService interface {
	...
	GetRoleUsernames(ctx context.Context, roleCode string) ([]string, error)
	...
	CreateSession(ctx context.Context, userID uint, ttl time.Duration) (*AccessSnapshot, error)
...
func NewRBACService(repo permission.AuthorizationRepository, roleRepo permission.RoleRepository, resourceRepo permission.ResourceRepository, userRepo user.Repository) RBACService {
	return &rbacService{repo: repo, roleRepo: roleRepo, resourceRepo: resourceRepo, userRepo: userRepo}
}
...
func (s *rbacService) GetRoleUsernames(ctx context.Context, roleCode string) ([]string, error) {
	role, err := s.roleRepo.GetByCode(ctx, canonicalRoleCode(roleCode))
	if err != nil {
		return nil, apperror.ErrRoleNotFound
	}
...
func (s *rbacService) CreateSession(ctx context.Context, userID uint, ttl time.Duration) (*AccessSnapshot, error) {
	if ttl <= 0 {
	...
	var active []uint
	now := time.Now()
	session := &permission.AuthorizationSession{ID: uuid.NewString(), UserID: userID, CreatedAt: now, ExpiresAt: now.Add(ttl), Version: 1}
	if err := s.repo.WithTx(ctx, func(tx permission.AuthorizationRepository) error {
		if err := tx.LockRoles(ctx); err != nil {
			return err
		}
