...
func NewRsaService(redisClient redis.UniversalClient, ns rediskey.Namespace, keys RsaKeyConfig) RsaService {
	svc := &rsaService{redisClient: redisClient, keys: ns}
...
func (svc *rsaService) GenerateKeyPair(ctx context.Context) (string, error) {
	...
	pipe.Set(ctx, svc.privateKeyKey(keyPairID), storedPrivateKey, RSAKeyExpiry+RSAKeyGracePeriod)
	pipe.Set(ctx, svc.publicKeyKey(keyPairID), string(publicKeyPEM), RSAKeyExpiry+RSAKeyGracePeriod)
	pipe.Set(ctx, svc.keys.Key(constant.RSA_CURRENT_KEY_PAIR_ID), keyPairID, RSAKeyExpiry)
...
func (svc *rsaService) RotateKeyPair(ctx context.Context) (string, error) {
	...
	currentKeyPairID, err := svc.redisClient.Get(ctx, svc.keys.Key(constant.RSA_CURRENT_KEY_PAIR_ID)).Result()
	...
	if currentKeyPairID != "" {
		...
		if err := svc.redisClient.Set(ctx, svc.keys.Key(RSAPreviousKeyPairID), currentKeyPairID, RSAKeyGracePeriod).Err(); err != nil {
...
func (svc *rsaService) GetPublicKey(ctx context.Context) (string, error) {
	keyPairID, err := svc.redisClient.Get(ctx, svc.keys.Key(constant.RSA_CURRENT_KEY_PAIR_ID)).Result()
	if err != nil {
		if err == redis.Nil {
			// 当前密钥对不存在或已过期，生成新密钥对
			return svc.GenerateKeyPair(ctx)
	...
	publicKey, err := svc.redisClient.Get(ctx, svc.publicKeyKey(keyPairID)).Result()
	if err != nil {
		if err == redis.Nil {
			// 公钥不存在，生成新密钥对
			return svc.GenerateKeyPair(ctx)
...
func (svc *rsaService) Decrypt(ctx context.Context, encryptedData string) (string, error) {
	...
	previousKeyPairID, err := svc.redisClient.Get(ctx, svc.keys.Key(RSAPreviousKeyPairID)).Result()
	if err != nil && err != redis.Nil {
...
func (svc *rsaService) privateKeyKey(keyPairID string) string {
	return svc.keys.Key(keyPairID + ":private")
}

func (svc *rsaService) publicKeyKey(keyPairID string) string {
	return svc.keys.Key(keyPairID + ":public")
}

func (svc *rsaService) decryptUsablePrivateKey(ctx context.Context, keyPairID string) (*rsa.PrivateKey, error) {
	storedPrivateKey, err := svc.redisClient.Get(ctx, svc.privateKeyKey(keyPairID)).Result()
	if err != nil {
