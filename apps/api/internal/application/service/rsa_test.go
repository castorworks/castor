...
func TestRsaServiceGetPublicKeyRegeneratesUnusableCurrentKeyPair(t *testing.T) {
	...
	oldSvc := NewRsaService(client, RsaKeyConfig{PrivateKeySecret: "old-rsa-secret"}).(*rsaService)
	...
	keyPairID, err := client.Get(ctx, constant.RSA_CURRENT_KEY_PAIR_ID).Result()
	if err != nil {
		t.Fatalf("get current key pair id: %v", err)
	}

	newSvc := NewRsaService(client, RsaKeyConfig{PrivateKeySecret: "new-rsa-secret"}).(*rsaService)
	...
	newKeyPairID, err := client.Get(ctx, constant.RSA_CURRENT_KEY_PAIR_ID).Result()
	if err != nil {
	...
	oldPrivateKey, err := client.Get(ctx, fmt.Sprintf("%s:private", keyPairID)).Result()
	if err != nil {
