...
func (m *mockUserRepo) GetByMobile(_ context.Context, mobile string) (*user.User, error) {
	if mobile == "" {
		return nil, shared.ErrNotFound
	}
	for _, u := range m.users {
...
func (m *mockUserRepo) CountByDepartment(context.Context) (map[uint]int64, error) {
	counts := map[uint]int64{}
	for _, u := range m.users {
		if u.DepartmentID != nil {
			counts[*u.DepartmentID]++
		}
	}
	return counts, nil
}

func (m *mockUserRepo) CountCreatedByMonth(_ context.Context, _ int, _ ...query.Option) ([]shared.MonthlyCount, error) {
...
type mockCaptchaClient struct {
	valid bool
	err   error // 模拟验证码存储（Redis）故障
}

func (m *mockCaptchaClient) Verify(_ context.Context, _, _ string) (bool, error) {
	return m.valid, m.err
}

func (m *mockCaptchaClient) Generate(context.Context) (string, string, error) {
	return "captcha-id", "captcha-image", m.err
}
...
func (m *mockAuthService) PostCode(_ context.Context, _ *dto.ConfirmCodeReq, _ string) error {
	return nil
}
...
func (m *mockLoginHistoryService) Gets(_ context.Context, _ permission.AccessScope, _, _ int, _ string, _ ...query.Option) ([]dto.LoginHistoryResp, int64, error) {
	return nil, 0, nil
}
...
func (m *mockLoginHistoryService) Delete(_ context.Context, _ permission.AccessScope, _ uint) error {
	return nil
}

func (m *mockLoginHistoryService) BatchDelete(_ context.Context, _ permission.AccessScope, _ []uint) error {
	return nil
}
...
// --- memDepartments implements department.Repository in memory ---

type memDepartments struct {
	items []department.Department
}

func (m *memDepartments) WithTx(ctx context.Context, fn func(department.Repository) error) error {
	return fn(m)
}
func (m *memDepartments) Lock(context.Context) error { return nil }
func (m *memDepartments) List(context.Context) ([]department.Department, error) {
	return slices.Clone(m.items), nil
}
func (m *memDepartments) Save(_ context.Context, d *department.Department) error {
	if d.ID == 0 {
		d.ID = uint(len(m.items) + 100)
...
func (m *memDepartments) Delete(_ context.Context, id uint) error {
	m.items = slices.DeleteFunc(m.items, func(d department.Department) bool { return d.ID == id })
	return nil
}
