...
func (*stubRBACService) EnsureCanManageUser(context.Context, uint, uint) error   { return nil }

// managerCtx 是管理端调用的上下文：用户管理服务要求能识别出调用者，
// 否则按越权处理（见 userService.ensureCanManage）。
func managerCtx() context.Context {
	return ucontext.WithAuditUserID(context.Background(), managerID)
}

// fullScope 是管理端"全部数据"范围：用户服务测试关注的不是数据范围时使用。
var fullScope = permission.AccessScope{All: true, UserID: managerID}

// managerID 不与任何测试夹具里的用户 ID 冲突，避免触发"不能修改自己"的规则。
const managerID uint = 9999
...
func TestUserService_Post_Validation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, err := svc.Post(context.Background(), fullScope, tt.req)
...
func TestUserService_Get_UserStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, err := svc.Get(context.Background(), fullScope, tt.userID)
			if (err != nil) != tt.wantErr {
...
func TestUserService_Delete(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := svc.Delete(managerCtx(), fullScope, tt.userID)
			if (err != nil) != tt.wantErr {
				t.Errorf("Delete() error = %v, wantErr %v", err, tt.wantErr)
			}
			// 用户删除成功后必须释放它引用的资产（头像），失败则不得触碰。
			if released := len(assets.detachedOwner) == 1 && assets.detachedOwner[0] == tt.userID; released == tt.wantErr {
				t.Errorf("DetachOwner calls = %v, wantErr %v", assets.detachedOwner, tt.wantErr)
			}

			if !tt.wantErr {
				// Verify user is actually deleted
				_, err := svc.Get(context.Background(), fullScope, tt.userID)
...
func TestNewUserServiceDoesNotInitializeDatabase(t *testing.T) {
	...
	NewUserService(repo, nil, nil, &stubRBACService{}, nil, &memDepartments{})
