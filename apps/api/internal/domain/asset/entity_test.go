...
func TestReference_Valid(t *testing.T) {
	tests := []struct {
		...
		{"complete", Reference{OwnerType: "user", OwnerID: 1, Field: "avatar"}, true},
		{"missing owner type", Reference{OwnerID: 1, Field: "avatar"}, false},
		{"missing owner id", Reference{OwnerType: "user", Field: "avatar"}, false},
		{"missing field", Reference{OwnerType: "user", OwnerID: 1}, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
