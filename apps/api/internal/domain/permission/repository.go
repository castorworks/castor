...
type AuthorizationRepository interface {
	...
	// SetRoleDataScope 更新角色的数据范围，并让 CUSTOM 的部门集合恰好等于 departmentIDs
	SetRoleDataScope(ctx context.Context, roleID uint, scope DataScope, departmentIDs []uint) error
	...
	// ListActiveSessions pages through sessions that are neither revoked nor expired.
	ListActiveSessions(ctx context.Context, page, size int, order string, opts ...query.Option) ([]AuthorizationSession, int64, error)
	...
	// PurgeSessions 删除在 before 之前就已撤销或过期的会话（含其激活角色），返回删除条数
