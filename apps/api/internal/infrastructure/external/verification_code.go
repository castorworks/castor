...
func NewVerificationCodeStore(rdb redis.UniversalClient) *VerificationCodeStore {
	cfg := config.C.VerifyCode
	return newVerificationCodeStore(rdb, time.Duration(cfg.ExpireSeconds)*time.Second, cfg.Length, cfg.MaxAttempts)
}

func newVerificationCodeStore(rdb redis.UniversalClient, ttl time.Duration, length, maxAttempts int) *VerificationCodeStore {
	if ttl < time.Second {
		ttl = 10 * time.Minute
	}
	...
	return &VerificationCodeStore{rdb: rdb, ttl: ttl, length: length, maxAttempts: int64(maxAttempts)}
}
...
func (s *VerificationCodeStore) keys(purpose, target string) (string, string) {
	...
	return verificationCodeKeyPrefix + suffix, verificationAttemptsKeyPrefix + suffix
}
