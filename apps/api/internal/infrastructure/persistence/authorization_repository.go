...
func (r *authorizationRepository) SetRoleDataScope(ctx context.Context, roleID uint, scope permission.DataScope, departmentIDs []uint) error {
	if err := r.q(ctx).Model(&models.RoleModel{}).Where("id = ?", roleID).UpdateColumn("data_scope", string(scope)).Error; err != nil {
		return err
	}
	return saveDepartments(r.q(ctx), &permission.Role{DataScope: scope, DepartmentIDs: departmentIDs}, roleID)
}
...
func (r *authorizationRepository) CreateSession(ctx context.Context, session *permission.AuthorizationSession, activeRoleIDs []uint) error {
	return r.q(ctx).Transaction(func(tx *gorm.DB) error {
		row := models.AuthorizationSessionModelFromEntity(session)
		...
		session.LastActiveAt = row.LastActiveAt
		return nil
	})
}
...
func (r *authorizationRepository) ListActiveSessions(ctx context.Context, page, size int, order string, opts ...query.Option) ([]permission.AuthorizationSession, int64, error) {
	opts = append(opts, *query.NewOption("revoked_at IS NULL AND expires_at > ?", time.Now()))
	rows, total, err := PaginatedQuery[models.AuthorizationSessionModel](ctx, r.db, page, size, order, opts...)
	if err != nil {
		return nil, 0, err
	}
	result := make([]permission.AuthorizationSession, len(rows))
...
func (r *authorizationRepository) TouchSession(ctx context.Context, id string, at time.Time) error {
	return r.q(ctx).Model(&models.AuthorizationSessionModel{}).Where("id = ? AND revoked_at IS NULL", id).UpdateColumn("last_active_at", at).Error
}
...
func (r *authorizationRepository) PurgeSessions(ctx context.Context, before time.Time) (int64, error) {
	// rbac_session_roles 随会话级联删除。
	result := r.q(ctx).Where("(revoked_at IS NOT NULL AND revoked_at < ?) OR expires_at < ?", before, before).
		Delete(&models.AuthorizationSessionModel{})
