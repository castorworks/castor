...
const (
	...
	defaultPageSize = 10
	maxPageSize     = 500
	// maxInValues caps the number of elements accepted in a `field-in=a,b,c` filter.
	maxInValues = 100
)
	...
	if len(params.QueryConditions) > 0 {
		...
		for _, condition := range params.QueryConditions {
			...
			case "in":
				...
				// Cap the IN list to bound query-build cost and placeholder count
				// against a hostile oversized `field-in=...` value.
				if len(values) > maxInValues {
					values = values[:maxInValues]
				}
				if len(values) > 0 {
...
// AllowedQueryFields 按字母序列出模型允许筛选、排序与搜索的列，供 OpenAPI 文档说明列表参数。
func AllowedQueryFields(modelType reflect.Type) []string {
	fields := make([]string, 0)
	for field := range getAllowedFields(modelType) {
