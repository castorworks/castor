...
const (
	...
	ErrCodeTOTPRequired        = 2012 // 需要两步验证码（登录响应 data.challenge 用于提交验证码）
	ErrCodeInvalidTOTP         = 2013 // 两步验证码或恢复码错误
	ErrCodeMFAChallengeExpired = 2014 // 两步验证登录已过期或尝试次数用尽，需重新登录
	...
	ErrCodeAssetInUse        = 4005 // 资产正被业务记录引用

	// === 5xxx: 系统配置错误 ===
	ErrCodeSettingNotFound       = 5001 // 配置项不存在
	ErrCodeSettingSaveFailed     = 5002 // 配置保存失败
	ErrCodeDeliveryNotConfigured = 5003 // 验证码投递通道（短信/邮件）未配置
