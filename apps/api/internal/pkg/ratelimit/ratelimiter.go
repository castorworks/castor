...
func NewRateLimiter(redisClient redis.UniversalClient, ns rediskey.Namespace) *RateLimiter {
	return &RateLimiter{redisClient: redisClient, keys: ns}
}
...
func (r *RateLimiter) IsRequestReachLimit(ctx context.Context, key string, duration time.Duration, maxAllowed int64) (bool, error) {
	cacheKey := r.keys.Key("rate_limit:" + key)
