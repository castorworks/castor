...
function typedField<TValues extends Record<string, unknown>>() {
  // `never` props：组件参数是逆变的，写成 `{ name: string }` 会把带必填 props（如 label）的字段拒之门外。
  return function <C extends React.ComponentType<never>>(Component: C): WithTypedName<C, TValues> {
