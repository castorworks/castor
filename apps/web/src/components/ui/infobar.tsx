...
// 与侧栏（16rem）并排放下 22rem 的面板还要留出内容区，需要至少 1280px（Tailwind xl）；
// 更窄时改为覆盖在内容上的抽屉，否则内容区被挤没、面板被裁到屏幕外。
const INFOBAR_INLINE_BREAKPOINT = 1280;
...
function InfobarProvider({
  ...
  const isMobile = useIsMobile(INFOBAR_INLINE_BREAKPOINT);
  ...
  return (
    ...
    >
      <div
        ...
        className={cn(
          'sticky top-0 z-30 hidden h-[calc(100dvh-3.5rem)] w-(--infobar-width) shrink-0 overflow-hidden rounded-tl-xl border-l border-t transition-[width,opacity] duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] xl:flex',
