...
export default function ProfilePage() {
  ...
  const nameMutation = useMutation({
    ...mergeMutationOptions(updateAccountNameMutation, {
      ...
      onError: (error) => toast.error(error.message || t('messages.nameUpdateFailed'))
    })
  });
  ...
  const passwordMutation = useMutation({
    ...mergeMutationOptions(updateAccountPasswordMutation, {
      ...
      onError: (error) => toast.error(error.message || t('messages.passwordUpdateFailed'))
    })
  });
  ...
  const accountSourceLabel = dict.label('account_source', accountInfo.accountSource);
  ...
  return (
    <div className='flex flex-col gap-4'>
        ...
        <CardContent className='flex flex-col gap-5 py-5 lg:flex-row lg:items-center lg:justify-between'>
          <div className='flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center'>
            <div className='group relative shrink-0'>
              ...
              >
                <AvatarImage
                  src={accountInfo.avatar ? assetUrl(accountInfo.avatar) : undefined}
      ...
      <ContactSection accountInfo={accountInfo} />
      <TwoFactorSection />
      <LinkedAccountsSection />
      <NotificationPreferencesSection accountInfo={accountInfo} />
...
function PasswordForm({ onSubmit, isPending }: PasswordFormProps) {
  ...
  const form = useAppForm({
    ...
    validators: {
      onSubmit: getUpdatePasswordSchema(policy, {
        currentRequired: t('validation.currentPasswordRequired'),
        mismatch: ta('passwordMismatch'),
        password: messages
      })
    },
