...
// ============================================================
// Read API for rendering enum labels
// ============================================================

/**
 * Fetch every enabled dictionary grouped by type code (requires sign-in).
 *
 * The unauthenticated `/v1/dictionaries` endpoint only serves types flagged
 * `isPublic`; nothing in the dashboard should depend on it.
 */
export async function getEnabledDicts(options?: RequestInit): Promise<DictMap> {
  return apiClient<DictMap>('/v1/account/dictionaries', options);
}
