...
/** 图标按 Icons 实际渲染出来，运营才能看出填的 key 对不对。 */
function DictItemIcon({ name }: { name: string }) {
  const Icon = Object.hasOwn(Icons, name) ? Icons[name as keyof typeof Icons] : undefined;
  return (
    <div className='flex items-center gap-1 text-muted-foreground'>
      {Icon ? <Icon className='h-3.5 w-3.5' /> : <Icons.warning className='h-3.5 w-3.5' />}
        ...
        <div className='flex items-center justify-between border-b px-5 py-3'>
          <div>
            <div className='flex items-center gap-2'>
              <h3 className='text-sm font-semibold'>{localizedText(type.name, locale)}</h3>
              {type.isSystem && (
                ...
                {items.map((item) => (
                  ...
                  >
                    <td className='px-5 py-2.5'>
                      <div className='flex items-center gap-2'>
                        <span className='font-medium'>{localizedText(item.label, locale)}</span>
                        {item.isSystem && (
                      ...
                      <div className='flex items-center gap-3'>
                        {item.color && (
                          <div className='flex items-center gap-1.5'>
                            <span
                              className={cn('h-3 w-3 rounded-full border', tagColorBg(item.color))}
                            />
                            <span className='text-xs text-muted-foreground'>
                              {tagColorBg(item.color) ? t(`colors.${item.color}`) : item.color}
                            </span>
                          </div>
                        )}
                        {item.icon && <DictItemIcon name={item.icon} />}
                        ...
                        <div className='flex items-center justify-end gap-0.5 opacity-0 transition-opacity group-hover:opacity-100'>
                          ...
                          {canDelete && (
                            <Button
                              ...
                              // 系统项被代码引用，只能停用；后端同样会拒绝
                              disabled={item.isSystem}
                              title={item.isSystem ? t('item.systemValueLocked') : undefined}
