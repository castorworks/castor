...
import {
  fromI18nText,
  getCreateDictTypeSchema,
  toI18nText,
  type CreateDictTypeFormValues
...
export function DictTypeFormSheet({ open, onOpenChange, existing }: DictTypeFormSheetProps) {
  ...
  const form = useAppForm({
    defaultValues: {
      code: existing?.code ?? '',
      ...fromI18nText(existing?.name),
      description: existing?.description ?? '',
      isPublic: existing?.isPublic ?? false,
      sortOrder: existing?.sortOrder ?? 0
    } as CreateDictTypeFormValues,
    validators: {
      onSubmit: getCreateDictTypeSchema(validationMessages)
    },
    ...
    <Sheet key={existing?.id ?? 'create'} open={open} onOpenChange={onOpenChange}>
      <SheetContent className='flex flex-col'>
        <SheetHeader>
          ...
          <SheetDescription>
            {isEdit
              ? t('type.editExisting', { name: localizedText(existing?.name, locale) })
              : t('type.newDescription')}
          </SheetDescription>
        </SheetHeader>
          ...
          <form.AppForm>
            <form.Form id='dict-type-form-sheet' className='space-y-4'>
              ...
              <form.SwitchField
                ...
                description={t('fields.isPublicDescription')}
              />
              <form.TextField
                name='sortOrder'
