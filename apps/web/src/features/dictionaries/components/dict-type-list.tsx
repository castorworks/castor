...
export function DictTypeList({
  ...
  const filteredTypes = useMemo(() => {
    ...
    // 按当前语言的名称搜索，同时保留按 code 搜索。
    return types.filter(
      (t) =>
        localizedText(t.name, locale).toLowerCase().includes(q) || t.code.toLowerCase().includes(q)
    );
  }, [types, search, locale]);
              ...
              >
                <div className='flex flex-col gap-0.5 min-w-0'>
                  <div className='flex items-center gap-1.5'>
                    ...
                    {type.isPublic && (
                      <Badge variant='info' className='h-4 shrink-0 px-1 text-2xs'>
                        {t('type.public')}
                      </Badge>
                    )}
                    {!type.isEnabled && (
