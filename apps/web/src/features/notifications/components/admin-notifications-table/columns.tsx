...
export function useAdminNotificationColumns(): ColumnDef<Notification>[] {
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => <DictBadge type='notification_type' value={row.original.type} />,
      enableColumnFilter: true,
      meta: {
        label: t('table.type'),
        variant: 'multiSelect' as const,
        get options() {
          return dictOptionsOr(dict, 'notification_type', [
    ...
    },
    {
      ...
      cell: ({ row }) => <DictBadge type='notification_level' value={row.original.level} />,
      enableColumnFilter: true,
      meta: {
        label: t('table.level'),
        variant: 'multiSelect' as const,
        get options() {
          return dictOptionsOr(dict, 'notification_level', [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className='grid min-w-48 grid-cols-2 gap-x-3 gap-y-1 text-xs'>
            ...
            {n.emailStats && (
              <>
                <span className='text-muted-foreground'>{t('table.emailsSent')}</span>
                <span className='text-right tabular-nums'>
                  {n.emailStats.sent}
                  {n.emailStats.pending > 0 && (
                    <span className='text-muted-foreground'> +{n.emailStats.pending}</span>
                  )}
                </span>
                {n.emailStats.failed > 0 && (
                  <>
                    <span className='text-destructive'>{t('table.emailsFailed')}</span>
                    <span className='text-destructive text-right tabular-nums'>
                      {n.emailStats.failed}
                    </span>
                  </>
                )}
