...
/** Set a role's data scope; CUSTOM needs at least one department */
export async function setRoleDataScope(
  code: string,
  dataScope: DataScope,
  departmentIds: number[]
): Promise<Role> {
  return apiClient<Role>(`/v1/admin/roles/${encodeURIComponent(code)}/data-scope`, {
    method: 'PUT',
    body: JSON.stringify({ dataScope, departmentIds: dataScope === 'CUSTOM' ? departmentIds : [] })
  });
}
