...
export function RoleCard({ role }: RoleCardProps) {
  ...
  // 能看角色列表即可查看数据范围；是否可改由面板内的权限与系统角色判断。
  ...
  return (
    <>
          ...
          <div className='flex flex-wrap items-center justify-between gap-3'>
            <div className='flex flex-wrap items-center gap-3 text-sm text-muted-foreground'>
              ...
              <span className='flex items-center gap-1.5'>
                <span className='text-xs'>{t('card.dataScope')}</span>
                <DictBadge type='role_data_scope' value={role.dataScope} />
              </span>
            </div>
              ...
              {hasMoreActions && (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align='end'>
                    ...
                    <DropdownMenuItem onSelect={() => setDataScopeOpen(true)}>
                      <Icons.employee /> {t('card.dataScope')}
                    </DropdownMenuItem>
      ...
      <RoleDataScopeSheet role={role} open={dataScopeOpen} onOpenChange={setDataScopeOpen} />
    </>
  );
}
