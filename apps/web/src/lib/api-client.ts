...
const CASTOR_UNAUTHORIZED_CODE = 2000;
...
async function isAuthenticationFailure(res: Response): Promise<boolean> {
  ...
  try {
    ...
    return body.code === CASTOR_UNAUTHORIZED_CODE || body.code === 401;
  } catch {
    return false;
  }
}
...
export async function readCastorResponse<T>(res: Response): Promise<T> {
  ...
  if (!res.ok) {
    ...
    if (body) {
      throw new CastorApiError(getErrorMessage(body.code, body.message), body.code, res.status);
    }
