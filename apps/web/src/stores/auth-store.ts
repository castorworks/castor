...
export interface LoginParams {
  ...
  captchaId?: string;
  captchaCode?: string;
  /** Keep the session across browser restarts (persistent cookie for the refresh window). */
  rememberMe?: boolean;
}
...
interface AuthState {
  ...
  /** Second sign-in step: the challenge from a 2012 error plus an authenticator or recovery code. */
  loginWithTOTP: (challenge: string, code: string) => Promise<void>;
  logout: () => Promise<void>;
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  loginWithTOTP: async (challenge, code) => {
    set({ isSubmitting: true, error: null });
    try {
      const data = await apiClient<LoginResponse>('/v1/auth/login/totp', {
        method: 'POST',
